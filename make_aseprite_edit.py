...
・bg / window / garland / light は1枚のレイヤー
・furniture と props は「グループ」にして、その中にプロップ1個ずつのレイヤーを入れる
・重ねた結果が room_edit.png と1画素も違わないことを確認してから保存する
...
LAY = os.path.join(HERE, "room_edit_layers")
OUT = os.path.join(HERE, "room_edit.aseprite")
...
room = Image.open(os.path.join(HERE, "room_edit.png")).convert("RGB")
diff = ImageChops.difference(flat.convert("RGB"), room)
bbox = diff.getbbox()
if bbox is None:
    print("検証OK: 重ねた結果は room_edit.png と完全一致")
else:
