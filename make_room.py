...
obj('おばけのパッチの絵本')
# ふだんは白い札に「PICTURE BOOK」とだけ。カーソルを合わせると本物の表紙(ehon_cover.png)が出る。
# 24ドットの正方形に 7文字は並ばないので、本の表紙と同じように行を割って読ませる
BOOKFONT = {
    'B': ("##.", "#.#", "##.", "#.#", "##."),
    'C': ("###", "#..", "#..", "#..", "###"),
    'E': ("###", "#..", "##.", "#..", "###"),
...
BOOKBOX = (74, 84, 24, 24)
O('furniture', BOOKBOX[0], BOOKBOX[1], BOOKBOX[2], BOOKBOX[3], 'wht', 'ink')
for _row, _line in enumerate(("PIC-", "TURE", "BOOK")):
    _tw = len(_line) * 4 - 1                           # 3ドットの字 + 1ドットのすき間
    _tx = BOOKBOX[0] + (BOOKBOX[2] - _tw) // 2
    _ty = BOOKBOX[1] + 2 + _row * 7
    for _i, _ch in enumerate(_line):
        for _dy, _grow in enumerate(BOOKFONT[_ch]):
            for _dx, _v in enumerate(_grow):
                if _v == '#':
                    PXL('furniture', _tx + _i * 4 + _dx, _ty + _dy, 'ink')
R('furniture', 77, 105, 18, 1, 'gray1')                # 下の小口(本らしさ)
